...
class MyHashMap:
    ...
    def print(self) -> str:
        #arr = [self.arr[i].get(k) for i in range(0,self.size) for k in range(0,self.arr[i]) if self.arr[i].get() != []]
        temparr = [self.arr[i].serialize() for i in range(0,self.size) if self.arr[i].serialize() != []]
